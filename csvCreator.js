...
const startBlock = 32533130; // Replace with the nonce value you want to start from
...
async function getUSDTTransferEvents() {
  ...
  const blockNumbers = await getBlockNumbersForDayWeekMonthYear(latestBlockNumber);
  const getPreviousBlock = latestBlockNumber - 1; // can be used to get all transactions per block 


  // Block range variables
  const batchSize = 10000;
  let fromBlock = 32533130; // startBlock; >> for first transaction tracked block > 32533130  
...
async function main() {
  try {
    const transactions = await getUSDTTransferEvents();
